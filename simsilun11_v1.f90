...
integer I,Ii  !increment counter
...
integer, parameter :: Ni = 100  ! dimension of the initial data vector - for single value Ni = 1
      double precision Din(Ni),dini ! initial density contrast
      double precision Rout(Ni), Reds(Ni)
! final density in Silent Universe and within linearly perturbed Einstein-de Sitter model
! Note difference between density and density contrast.

!These must sync with the other programme
...
! load initial data: density contrast vector deli(Ni) and other data in InD(10)
	call initial_data(cpar,InD,Ni,Rin,Din)


	Rout = 0.0d0
	Reds = 0.0d0
! calculate the evolution of X(Nx) -> then -> write density to Xout
!$OMP PARALLEL DO PRIVATE(I,dini,X),SHARED(InD,Din,Rin,Rout,Reds)
	do I=1,Ni
	   dini = Din(I)
	   call silent_evolution(InD,dini,Nx,X)
     Rin(I) = I*1d0
	   Rout(I) = (X(1)/InD(5)) - 1
	   Reds(I) = Din(I)*(InD(2)/InD(5))**(1.0/3.0)
	enddo
!$OMP END PARALLEL DO
